...
def xopp_rectangle(
    x1, y1, x2, y2, tool="pen", ts="0ll", fn="", color="#000000ff", line_width=2.26
):
...
def xopp_text(text, x, y, size=12, color="#000000ff", font="Sans", ts="0ll", fn=""):
...
def xopp_score_table(root, width, tag, scores):
    ...
    for i, name in enumerate(scores):
        lx = x1 + (i + 1) * h
        layer.insert(
            k, xopp_line(lx, y1, lx, y2, color="#3333ccff", line_width=2.26 * scl)
        )
        k += 1
        layer.insert(
            k, xopp_text(name, lx - 0.55 * h, y1 + 0.25 * h0, size=int(12 * scl))
        )
        k += 1
        layer.insert(
            k, xopp_text(str(scores[name]), lx - 0.6 * h, ly + 0.4 * h1, int(12 * scl))
        )
        k += 1
        summ += scores[name]

    layer.insert(k, xopp_summ(x1 + (n + 0.4) * h, y1 + 0.02 * h0, width=27.674 * scl))
    k += 1
    layer.insert(
        k, xopp_text(str(summ), x1 + (n + 0.4) * h, ly + 0.4 * h1, int(12 * scl))
    )
...
def search_annotations(prefix):
    ...
    for tag in result:
        result[tag] = sorted(
            result[tag], key=lambda x: int(re.match(mtemp, x).group(2))
        )
...
if __name__ == "__main__":
    ...
    for tag in annotations:
        ...
        scores[tag] = xopps_merge(
            tag, annotations[tag], xopp_path, os.path.abspath(args.pdf_prefix), args.scoring
        )
        if args.pdf_export_prefix is not None:
